...
class ASTVisitor(VtlVisitor):
    ...
    def visitStart(self, ctx: Parser.StartContext):
        ...
        statements = [
            ...
            if isinstance(statement, Parser.StatementContext)
        ]
        if len(statements) != 0:
    ...
    def visitDefDatapointRuleset(self, ctx: Parser.DefDatapointRulesetContext):
        ...
        return DPRuleset(
            name=ruleset_name, params=ruleset_elements, rules=ruleset_rules
        )
    ...
    def visitRuleItemDatapoint(self, ctx: Parser.RuleItemDatapointContext):
        ...
        when = [
            ...
            if isinstance(whens, TerminalNodeImpl)
            and whens.getSymbol().type == Parser.WHEN
        ]
        rule_name = [
            (
        ...
        if len(when) != 0:
            rule_node = HRBinOp(
                left=expr_node[0], op=when[0].getSymbol().text, right=expr_node[1]
            )
    ...
    def visitParameterItem(self, ctx: Parser.ParameterItemContext):
        ...
        return Argument(
            name=argument_name.value, type_=argument_type, default=argument_default
        )
    ...
    def visitDefHierarchical(self, ctx: Parser.DefHierarchicalContext):
        ...
        return HRuleset(
            name=ruleset_name, element=ruleset_elements, rules=ruleset_rules
        )
    ...
    def visitHierRuleSignature(self, ctx: Parser.HierRuleSignatureContext):
        ...
        if conditions:
            ...
            identifiers_list.append(
                DefIdentifier(value=dataset.getSymbol().text, kind=kind)
            )
    ...
    def visitCodeItemRelation(self, ctx: Parser.CodeItemRelationContext):
        ...
        items = [
            ...
            if isinstance(item, Parser.CodeItemRelationClauseContext)
        ]
