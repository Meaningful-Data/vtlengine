...
class TestParameterizedOperations:
    ...
    def test_round_dataset_operation(self):
        ...
        expected_sql = (
            'SELECT "Id_1", '
            'vtl_round_sig(ROUND(CAST("Me_1" AS DOUBLE), COALESCE(CAST(2 AS INTEGER), 0)), 15) '
            'AS "Me_1", '
            'vtl_round_sig(ROUND(CAST("Me_2" AS DOUBLE), COALESCE(CAST(2 AS INTEGER), 0)), 15) '
            'AS "Me_2" FROM "DS_1"'
        )
        assert_sql_equal(sql, expected_sql)
...
class TestTimeOperators:
    ...
    # NOTE: Tests for period_indicator, flow_to_stock, stock_to_flow, and
    # duration conversions are deferred to #519: (Duckdb) Implement time operators.
