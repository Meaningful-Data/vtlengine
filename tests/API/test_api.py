...
from vtlengine.API._InternalApi import load_vtl, load_value_domains, load_datasets, \
...
load_datasets_input_params_OK = [
    (filepath_json / 'DS_1.json'),
    ({"datasets": [{"name": "DS_1",
                    "DataStructure": [{"name": "Id_1", "role": "Identifier", "type": "Integer",
                                       "nullable": False},
                                      {"name": "Id_2", "role": "Identifier", "type": "String",
                                       "nullable": False},
                                      {"name": "Me_1", "role": "Measure", "type": "Number",
                                       "nullable": True}]}]})
]
...
load_datasets_with_data_without_dp_params_OK = [
    (filepath_json / 'DS_1.json', None, ({'DS_1': Dataset(name="DS_1", components={
        'Id_1': Component(name='Id_1', data_type=DataTypes.Integer, role=Role.IDENTIFIER,
                          nullable=False),
        'Id_2': Component(name='Id_2', data_type=DataTypes.String, role=Role.IDENTIFIER,
                          nullable=False),
        'Me_1': Component(name='Me_1', data_type=DataTypes.Number, role=Role.MEASURE,
                          nullable=True)},
                                                          data=pd.DataFrame(
...
load_datasets_with_data_path_params_OK = [
    (filepath_json / 'DS_1.json', filepath_csv / 'DS_1.csv',
...
load_datasets_with_data_and_wrong_inputs = [
    (filepath_csv / 'DS_1.csv', filepath_csv / 'DS_1.csv',
...
def test_load_external_routine(input):
    ...
    reference = {
        '1': ExternalRoutine(dataset_names=['BNFCRS_TRNSFRS', 'BNFCRS_TRNSFRS_CMMN_INSTRMNTS_4'],
...
@pytest.mark.parametrize('script, data_structures, value_domains, external_routines',
                         params_semantic)
    ...
    reference = {'DS_r': Dataset(name="DS_r", components={
        ...
        'Me_1': Component(name='Me_1', data_type=DataTypes.Number, role=Role.MEASURE,
                 ...
                 'DS_r2': Dataset(name="DS_r2", components={
                     'Id_1': Component(name='Id_1', data_type=DataTypes.Integer,
                                       role=Role.IDENTIFIER, nullable=False),
                     'Id_2': Component(name='Id_2', data_type=DataTypes.String,
                                       role=Role.IDENTIFIER, nullable=False),
                     'Me_1': Component(name='Me_1', data_type=DataTypes.Number, role=Role.MEASURE,
                                       nullable=True)},
                                  data=pd.DataFrame(columns=["Id_1", "Id_2", "Me_1"], index=[0, 1],
