...
from vtlengine.Model import Dataset, Component, DataComponent, Scalar, ScalarSet, ValueDomain, ExternalRoutine, Role
...
params: List[Tuple[type, Dict[str, Any]]] = [
    ...
    (Component, {"name": "id_int", "data_type": DataTypes.Integer, "role": Role.IDENTIFIER, "nullable": False}),
    (Component, {"name": "me_num", "data_type": DataTypes.Number, "role": Role.MEASURE, "nullable": True}),
    (Component, {"name": "attr_str", "data_type": DataTypes.String, "role": Role.ATTRIBUTE, "nullable": True}),
    (Component, {"name": "", "data_type": DataTypes.Boolean, "role": Role.MEASURE, "nullable": True}),

    (DataComponent, {"name": "data_comp", "data": None, "data_type": DataTypes.Number, "role": Role.MEASURE, "nullable": True}),
    (DataComponent, {"name": "id_dc", "data": [1, 2, 3], "data_type": DataTypes.Integer, "role": Role.IDENTIFIER, "nullable": False}),
    (DataComponent, {"name": "attr_dc", "data": ["A", "B"], "data_type": DataTypes.String, "role": Role.ATTRIBUTE, "nullable": True}),
    (DataComponent, {"name": "flag_dc", "data": [True, False], "data_type": DataTypes.Boolean, "role": Role.MEASURE, "nullable": True}),
    ...
    (ExternalRoutine, {"dataset_names": ["t1", "t2"], "query": "SELECT * FROM t1 JOIN t2 ON 1=1", "name": "my_routine"}),
    (ExternalRoutine, {"dataset_names": ["sales"], "query": "SELECT * FROM sales WHERE amount > 0", "name": "r_sales"}),
    (ExternalRoutine, {"dataset_names": ["a", "b"], "query": "SELECT a.id, b.val FROM a JOIN b ON a.id = b.id", "name": "r_join"}),
]
