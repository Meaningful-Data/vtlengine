...
# ---------------------------------------------------------------------------
# 10. Input loading: datapoint dict key matched case-insensitively
# ---------------------------------------------------------------------------


def test_datapoints_dict_key_case_insensitive():
    # A datapoints dict keyed 'ds_1' must bind to structure 'DS_1'.
    result = _run("DS_r <- DS_1;", datapoints={"ds_1": BASE_DATAPOINTS["DS_1"]})
    assert len(result["DS_r"].data) == 3


def test_calc_mixed_case_ref_computes_values():
    # A mixed-case measure reference must actually feed the computation.
    result = _run("DS_r <- ds_1[calc Me_2 := ME_1 * 2];")
    assert sorted(result["DS_r"].data["Me_2"].tolist()) == [20.0, 40.0, 60.0]


# ---------------------------------------------------------------------------
# 11. semantic_analysis (structure-only public API)
# ---------------------------------------------------------------------------

semantic_params = [
    pytest.param("DS_r <- ds_1;", id="dataset_alias"),
    pytest.param("DS_r <- DS_1[filter ME_1 > 15];", id="component_ref"),
    pytest.param("DS_r <- DS_1; DS_r2 <- ds_r;", id="chained"),
]


@pytest.mark.parametrize("script", semantic_params)
def test_semantic_analysis_case_insensitive(script):
    result = semantic_analysis(script=script, data_structures=BASE_STRUCTURES)
    assert any(name.startswith("DS_r") for name in result)


def test_returned_components_are_plain_dict_for_external_consumers():
    # The internal CaseInsensitiveDict must not leak to callers.
    result = semantic_analysis(script="DS_r <- ds_1;", data_structures=BASE_STRUCTURES)
    assert type(result["DS_r"].components) is dict


# ---------------------------------------------------------------------------
# 12. Rename duplicate detection (case-insensitive)
# ---------------------------------------------------------------------------


def test_rename_duplicate_source_different_case_raises():
    # Me_1 and ME_1 are the same component; renaming it twice must be rejected.
    with pytest.raises(SemanticError, match="1-1-6-9"):
        _run("DS_r <- DS_1[rename Me_1 to A, ME_1 to B];")
