...
def binary_implicit_promotion(
        left_type: ScalarType, right_type: ScalarType, type_to_check:ScalarType=None, return_type:ScalarType=None
    ) -> ScalarType:
    """
...
def check_binary_implicit_promotion(
    ...
    if type_to_check:
        if type_to_check.is_included(set_=left_implicities.intersection(right_implicities)):
            return True
        return False
    
    if left.is_included(right_implicities) or right.is_included(left_implicities):
        return True
    return False
