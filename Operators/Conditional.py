...
class If(Operator):
    ...
    def evaluate(cls, condition, true_branch, false_branch):
        ...
        if isinstance(condition, Dataset):
            result = cls.dataset_level_evaluation(
        ...
        for i, row in enumerate(condition.data):
            if row:
                data.append(
                    ...
                    else true_branch.data[i]
                )
            else:
                data.append(
                    false_branch.value
                    if isinstance(false_branch, Scalar)
                    else false_branch.data[i]
                )
    ...
    def dataset_level_evaluation(cls, result, condition, true_branch, false_branch):
        ...
        if isinstance(result, Dataset):
            drop_columns = [
                ...
                if column not in result.components.keys()
            ]
    ...
    def validate(
        ...
        if true_branch.__class__ != false_branch.__class__:
            if (
                isinstance(true_branch, DataComponent)
                and isinstance(false_branch, Dataset)
            ) or (
                isinstance(true_branch, Dataset)
                and isinstance(false_branch, DataComponent)
            ):
                raise ValueError(
        ...
        if isinstance(left, DataComponent):
            raise ValueError(
                "If operation at dataset level cannot have component type on left (condition) side"
            )
        if isinstance(left, Scalar):
            ...
            return Dataset(
                name="result", components=condition.components.copy(), data=None
            )
        ...
        if isinstance(right, Dataset):
            if left.components != right.components:
                raise ValueError(
                    "If operands at dataset level must have the same components"
                )
            for component in left.get_measures():
        ...
        if isinstance(condition, Dataset):
            if (
                ...
                or condition.get_measures()[0].data_type != Boolean
            ):
                raise ValueError(
                    "If operation at dataset level condition side must be a dataset having an unique "
                    "boolean measure"
                )
            if left.get_identifiers() != condition.get_identifiers():
                raise ValueError(
                    "If operands at dataset level must have the same identifiers as condition"
                )
        result_components = {
            comp_name: copy(comp) for comp_name, comp in left.components.items()
        }
    ...
    def validate(cls, left, right) -> Scalar | DataComponent | Dataset:
        ...
        if isinstance(left, Dataset):
            ...
            if isinstance(right, Scalar):
                for component in left.get_measures():
                    if component.data_type != right.data_type:
                        component.data_type = cls.type_validation(
