...
class Operator:
    ...
    def validate_type_compatibility(cls, *args) -> bool:
        if len(args) == 1:
            operand = args[0]
            return check_unary_implicit_promotion(
                operand, cls.type_to_check, cls.return_type
            )
        if len(args) == 2:
            left, right = args
            return check_binary_implicit_promotion(
                left, right, cls.type_to_check, cls.return_type
            )
        raise Exception("Method should be implemented by inheritors")
    ...
    def type_validation(cls, *args) -> ScalarType:
        ...
        if len(args) == 2:
            left, right = args
            return binary_implicit_promotion(
                left, right, cls.type_to_check, cls.return_type
            )
        raise Exception("Method should be implemented by inheritors")
...
class Binary(Operator):
    ...
    @classmethod
    # flake8: noqa
    def validate(cls, left: ALL_MODEL_DATA_TYPES, right: ALL_MODEL_DATA_TYPES):
        """
        ...
        if isinstance(left, Dataset) and isinstance(right, Dataset):
            return cls.dataset_validation(left, right)
        if isinstance(left, Dataset) and isinstance(right, Scalar):
            return cls.dataset_scalar_validation(left, right)
        if isinstance(left, Scalar) and isinstance(right, Dataset):
            return cls.dataset_scalar_validation(right, left)
        if isinstance(left, Scalar) and isinstance(right, Scalar):
            return cls.scalar_validation(left, right)

        if isinstance(left, DataComponent) and isinstance(right, DataComponent):
            return cls.component_validation(left, right)

        if isinstance(left, DataComponent) and isinstance(right, Scalar):
            return cls.component_scalar_validation(left, right)

        if isinstance(left, Scalar) and isinstance(right, DataComponent):
            return cls.component_scalar_validation(right, left)

        # In operator

        if isinstance(left, Dataset) and isinstance(right, ScalarSet):
            return cls.dataset_set_validation(left, right)

        if isinstance(left, Scalar) and isinstance(right, ScalarSet):
            return cls.scalar_set_validation(left, right)

        if isinstance(left, DataComponent) and isinstance(right, ScalarSet):
            return cls.component_set_validation(left, right)

    @classmethod
    def dataset_validation(
        cls, left_operand: Dataset, right_operand: Dataset
    ) -> Dataset:
        ...
        for left_measure, right_measure in zip(left_measures, right_measures):
            if not cls.validate_type_compatibility(
                left_measure.data_type, right_measure.data_type
            ):
                raise Exception(
                    f"{left_measure.name} with type {left_measure.data_type}, "
                    f"{right_measure.name} with type {right_measure.data_type}"
        ...
        for comp in [x for x in result_components.values() if x.role == Role.MEASURE]:
            if (
                comp.name in left_operand.components
                and comp.name in right_operand.components
            ):
    ...
    def scalar_validation(cls, left_operand: Scalar, right_operand: Scalar) -> Scalar:
        if not cls.validate_type_compatibility(
            left_operand.data_type, right_operand.data_type
        ):
            raise Exception(
                f"{left_operand.name} with type {left_operand.data_type} and "
                f"{right_operand.name} with type {right_operand.data_type} "
                f"is not compatible with {cls.op}"
            )

        return Scalar(
            name="result",
            data_type=cls.type_validation(
                left_operand.data_type, right_operand.data_type
            ),
            value=None,
        )
    ...
    def component_validation(
        ...
        """Validates the compatibility between the types of the components
        and the operator
        ...
        """
        # We can omit the first validation because
        # we check again in the next line
        if not cls.validate_type_compatibility(
            left_operand.data_type, right_operand.data_type
        ):
            raise Exception(
        ...
        result_data_type = cls.type_validation(
            left_operand.data_type, right_operand.data_type
        )
    ...
    def component_scalar_validation(cls, component: DataComponent, scalar: Scalar):
        if not cls.validate_type_compatibility(component.data_type, scalar.data_type):
            raise Exception(
                ...
                f"and {scalar.name} with type {scalar.data_type} "
                f"is not compatible with {cls.op}"
            )
    ...
    def dataset_set_validation(cls, dataset: Dataset, scalar_set: ScalarSet) -> Dataset:
        ...
        for measure in dataset.get_measures():
            if not cls.validate_type_compatibility(
                measure.data_type, scalar_set.data_type
            ):
                raise Exception(
                    f"{measure.name} with type {measure.data_type} "
                    f"and scalar_set with type {scalar_set.data_type} "
                    f"is not compatible with {cls.op}"
                )
    ...
    def component_set_validation(
        ...
        if not cls.validate_type_compatibility(
            component.data_type, scalar_set.data_type
        ):
            raise Exception(
                f"{component.name} with type {component.data_type} "
                f"and scalar_set with type {scalar_set.data_type} "
    ...
    @classmethod
    def type_validation(
        cls, left_type: ScalarType, right_type: ScalarType
    ) -> ScalarType:
        """
        Validates the compatibility between the types of the operands and
        the operator and give us the result ScalarType of the promotion
        ...
        """
        return binary_implicit_promotion(
            left_type, right_type, cls.type_to_check, cls.return_type
        )
            ...
            if is_mono_measure and left_type.promotion_changed_type(result_data_type):
                ...
                if result_dataset.data is not None:
                    result_dataset.data.rename(
                        columns={measure.name: component.name}, inplace=True
                    )
            elif (
    ...
    def dataset_evaluation(cls, left_operand: Dataset, right_operand: Dataset):
        ...
        if len(left_operand.get_identifiers_names()) < len(
        ...
        for measure_name in left_operand.get_measures_names():
            if use_right_as_base:
                result_data[measure_name] = cls.apply_operation_two_series(
                    result_data[measure_name + "_y"], result_data[measure_name + "_x"]
                )
            else:
                result_data[measure_name] = cls.apply_operation_two_series(
                    result_data[measure_name + "_x"], result_data[measure_name + "_y"]
                )
            result_data = result_data.drop(
                [measure_name + "_x", measure_name + "_y"], axis=1
            )

        # Delete attributes from the result data
        attributes = list(
            set(left_operand.get_attributes_names()).union(
                right_operand.get_attributes_names()
            )
        )
    ...
    def component_evaluation(
        ...
        result_component.data = cls.apply_operation_two_series(
            left_operand.data.copy(), right_operand.data.copy()
        )
    ...
    def component_scalar_evaluation(
        ...
        result_component.data = cls.apply_operation_series_scalar(
            component.data.copy(), scalar.value, component_left
        )
        return result_component
    ...
    def dataset_set_evaluation(cls, dataset: Dataset, scalar_set: ScalarSet) -> Dataset:
        ...
        for measure_name in dataset.get_measures_names():
            result_data[measure_name] = cls.apply_operation_two_series(
                dataset.data[measure_name], scalar_set.values
            )
    ...
    def component_set_evaluation(
        ...
        result_component.data = cls.apply_operation_two_series(
            component.data.copy(), scalar_set.values
        )
        return result_component
    ...
    def evaluate(
        ...
        if isinstance(left_operand, Dataset) and isinstance(right_operand, Scalar):
            return cls.dataset_scalar_evaluation(
                left_operand, right_operand, dataset_left=True
            )
        if isinstance(left_operand, Scalar) and isinstance(right_operand, Dataset):
            return cls.dataset_scalar_evaluation(
                right_operand, left_operand, dataset_left=False
            )

        if isinstance(left_operand, DataComponent) and isinstance(
        ...
        if isinstance(left_operand, DataComponent) and isinstance(
            ...
            return cls.component_scalar_evaluation(
                left_operand, right_operand, component_left=True
            )

        if isinstance(left_operand, Scalar) and isinstance(
            right_operand, DataComponent
        ):
            return cls.component_scalar_evaluation(
...
class Unary(Operator):
    ...
    @classmethod
    def apply_return_type_dataset(
        cls, result_dataset: Dataset, operand: Dataset
    ) -> None:
        changed_allowed = cls.op in MONOMEASURE_CHANGED_ALLOWED
        ...
        for measure in result_dataset.get_measures():
            ...
            if is_mono_measure and operand_type.promotion_changed_type(
                result_data_type
            ):
                component = Component(
                    name=COMP_NAME_MAPPING[result_data_type],
