...
class Calc:
    ...
    def validate(cls, operands: List[Union[DataComponent, Scalar]], dataset: Dataset):
        ...
        result_dataset = Dataset(
            name=dataset.name, components=dataset.components, data=None
        )
    ...
    def validate(cls, operands: List[str], dataset: Dataset):
        for operand in operands:
            if operand not in dataset.get_components_names():
                raise Exception(
                    f"Component {operand} not found in dataset {dataset.name}"
                )
            if dataset.get_component(operand).role == Role.IDENTIFIER:
                raise Exception(
                    f"Component {operand} in dataset {dataset.name} is an "
    ...
    def validate(cls, operands: List[RenameNode], dataset: Dataset):
        for operand in operands:
            if operand.old_name not in dataset.components.keys():
                raise Exception(
                    f"Component {operand.old_name} not found in dataset {dataset.name}"
                )
...
class Unpivot:
    ...
    def validate(cls, operands: List[str], dataset: Dataset):
        ...
        if identifier in dataset.components:
            raise ValueError(
                f"Component {identifier} already exists in dataset {dataset.name}"
            )

        result_components = {comp.name: comp for comp in dataset.get_identifiers()}
        result_dataset = Dataset(
            name=dataset.name, components=result_components, data=None
        )
        # noinspection PyTypeChecker
        result_dataset.add_component(
            Component(
                name=identifier, data_type=String, role=Role.IDENTIFIER, nullable=False
            )
        )
        base_type = None
        final_type = String
        for comp in dataset.get_measures():
            if base_type is None:
                ...
                if check_unary_implicit_promotion(base_type, comp.data_type) is None:
                    raise ValueError(
                        "All measures must have the same data type on unpivot clause"
                    )
            final_type = unary_implicit_promotion(base_type, comp.data_type)

        result_dataset.add_component(
            Component(
                name=measure, data_type=final_type, role=Role.MEASURE, nullable=True
            )
        )
        return result_dataset
...
class Sub:
    ...
    def validate(cls, operands: List[DataComponent], dataset: Dataset):
        for operand in operands:
            if operand.name not in dataset.components:
                raise Exception(
                    f"Component {operand.name} not found in dataset {dataset.name}"
                )
            if operand.role != Role.IDENTIFIER:
                raise Exception(
                    f"Component {operand.name} in dataset {dataset.name} is not an "
                    f"{Role.IDENTIFIER}"
                )
    ...
    def evaluate(cls, operands: List[DataComponent], dataset: Dataset):
        ...
        if len(dataset.data) > 0:
            ...
            for operand in operands:
                if is_first:
                    true_indexes = set(operand.data[operand.data == True].index)  # noqa
                    is_first = False
                else:
                    true_indexes.intersection_update(
                        set(operand.data[operand.data == True].index)  # noqa
                    )
