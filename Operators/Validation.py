...
class Check(Operator):
    ...
    def validate(
        ...
        if len(validation_element.get_measures()) != 1:
            raise Exception(
                "The validation operand must have exactly one measure of type Boolean"
            )
    ...
    def evaluate(
        ...
        columns_to_keep = (
            validation_element.get_identifiers_names()
        ...
        if invalid:
            ...
            result.data = result.data[
                result.data[validation_measure_name] == False  # noqa
            ]
...
class Check_Datapoint(Operator):
    ...
    @classmethod
    def validate(
        cls, dataset_element: Dataset, rule_info: Dict[str, Any], output: str
    ) -> Dataset:
        result_components = {
            comp.name: comp for comp in dataset_element.get_identifiers()
        }
    ...
    def evaluate(
        ...
        for rule_name, rule_data in rule_info.items():
            ...
            rule_df["errorcode"] = rule_df["bool_var"].map(
                {False: rule_data["error_code"]}
            )
            rule_df["errorlevel"] = rule_df["bool_var"].map(
                {False: rule_data["error_level"]}
            )
        ...
        if output == "invalid":
            ...
            result.data = result.data[
                result.get_identifiers_names() + ["bool_var", "errorcode", "errorlevel"]
            ]
        else:  # output == 'all_measures'
            result.data = result.data[
                result.get_identifiers_names()
                + dataset_element.get_measures_names()
                + ["bool_var", "errorcode", "errorlevel"]
            ]
