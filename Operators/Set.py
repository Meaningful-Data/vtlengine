...
class Set(Operator):
    ...
    def check_same_structure(cls, dataset_1: Dataset, dataset_2: Dataset) -> None:
        if len(dataset_1.components) != len(dataset_2.components):
            raise Exception(
                f"Datasets {dataset_1.name} and {dataset_2.name} "
        ...
        for comp in dataset_1.components.values():
            if comp.name not in dataset_2.components:
                raise Exception(
                    f"Component {comp.name} not found in dataset {dataset_2.name}"
                )
        ...
        for operand in operands:
            if len(result_components) == 0:
                ...
                for comp_name, comp in operand.components.items():
                    ...
                    result_components[comp_name].nullable = (
                        current_comp.nullable or comp.nullable
                    )
...
class Union(Set):
    ...
    def evaluate(cls, operands: List[Dataset]) -> Dataset:
        ...
        result.data = result.data.drop_duplicates(
            subset=identifiers_names, keep="first"
        )
    ...
    def evaluate(cls, operands: List[Dataset]) -> Dataset:
        ...
        for data in all_datapoints:
            if result.data is None:
                ...
                not_identifiers = [
                    col
                    for col in result.get_measures_names()
                ...
                result.data = result.data[
                    result.get_identifiers_names() + not_identifiers
                ]
        result.data.reset_index(drop=True, inplace=True)
        return result
    ...
    def evaluate(cls, operands: List[Dataset]) -> Dataset:
        ...
        for data in all_datapoints:
            if result.data is None:
                ...
                not_identifiers = (
                    result.get_measures_names() + result.get_attributes_names()
                )
                for col in not_identifiers:
    ...
    def evaluate(cls, operands: List[Dataset]) -> Dataset:
        ...
        for data in all_datapoints:
            if result.data is None:
                ...
                result.data = result.data.merge(
                    data, how="left", on=result.get_identifiers_names()
                )
