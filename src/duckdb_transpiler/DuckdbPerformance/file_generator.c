 ...
 * Supports optional row shuffling via Fisher-Yates algorithm.
 *
 * Compile: gcc -O3 -o file_generator file_generator.c -lm
 * Or just: make
 */
...
static inline void buf_put_char(Buffer *b, char c) {
    if (b->size + 1 > b->cap) buf_reserve(b, 1);
    b->data[b->size++] = c;
}

static inline void buf_write(Buffer *b, const char *s, size_t n) {
    buf_reserve(b, n);
...
// =============================================================================
// Random Number Generation (XorShift - faster than rand())
// =============================================================================

static inline uint32_t rand_xor_shift(uint32_t *s) {
    uint32_t x = *s;
    x ^= x << 13;
    x ^= x >> 17;
    x ^= x << 5;
...
static inline uint64_t rand_xor_shift64(uint64_t *s) {
    uint64_t x = *s;
    x ^= x << 13;
    x ^= x >> 7;
    x ^= x << 17;
...
static int starts_with_id(const char *s) {
    if (!s || !s[0]) return 0;
    return (tolower((unsigned char)s[0]) == 'i' && tolower((unsigned char)s[1]) == 'd');
}
...
static int parse_config(const char *path, Config *cfg) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!strncmp(line, "rows=", 5))
            cfg->rows = strtoull(line + 5, NULL, 10);
        else if (!strncmp(line, "chunk_size=", 11))
            cfg->chunk_size = strtoull(line + 11, NULL, 10);
        else if (!strncmp(line, "max_num=", 8))
            cfg->max_num = strtoull(line + 8, NULL, 10);
        else if (!strncmp(line, "max_str=", 8))
            cfg->max_str = atoi(line + 8);
        else if (!strncmp(line, "csv_path=", 9))
            strncpy(cfg->csv_path, line + 9, sizeof(cfg->csv_path) - 1);
        else if (!strncmp(line, "ds_path=", 8))
            strncpy(cfg->ds_path, line + 8, sizeof(cfg->ds_path) - 1);
        else if (!strncmp(line, "columns=", 8))
            cfg->columns = atoi(line + 8);
        else if (!strncmp(line, "shuffle=", 8))
            cfg->shuffle = atoi(line + 8);
        else if (!strncmp(line, "seed=", 5))
            cfg->seed = (uint32_t)strtoul(line + 5, NULL, 10);
    ...
    for (int i = 0; i < cfg->columns; ++i) {
        if (cfg->cols[i].type_code == TYPE_INVALID) return 0;
    }
...
// =============================================================================
// ID Plan (for generating unique identifiers)
// =============================================================================
...
static int build_id_plan(const Config *cfg, IdPlan *plan) {
    ...
    // Calculate factors for mixed-radix representation
    // We compute factors carefully to avoid overflow
    uint64_t prod = 1;
    int overflow = 0;
    for (int i = 0; i < plan->id_count; ++i) {
        ...
        if (prod > 0 && plan->bases[i] > UINT64_MAX / prod) {
            // Overflow would occur, but we might still have enough combinations
            // if we already exceed cfg->rows
            if (prod >= cfg->rows) {
                // We have enough, cap remaining factors
                overflow = 1;
            } else {
                // Check if we can still represent cfg->rows
                double total_est = (double)prod * (double)plan->bases[i];
                if (total_est < (double)cfg->rows) {
                    fprintf(stderr, "Cannot generate %llu unique rows. Estimated max: %.0f\n",
                            (unsigned long long)cfg->rows, total_est);
                    return 0;
                }
                overflow = 1;
            }
    ...
    // Final check: verify we have enough combinations
    if (!overflow) {
        if (cfg->rows > prod) {
            fprintf(stderr, "Cannot generate %llu unique rows with defined ids. Max: %llu\n",
                    (unsigned long long)cfg->rows, (unsigned long long)prod);
            return 0;
        }
    }
...
// =============================================================================
// Shuffle Index Generation (Fisher-Yates)
// =============================================================================

static uint64_t* generate_shuffle_indices(uint64_t n, uint32_t seed) {
    uint64_t *indices = (uint64_t*)malloc(sizeof(uint64_t) * n);
    if (!indices) {
        fprintf(stderr, "Cannot allocate shuffle indices for %llu rows\n",
    ...
    uint64_t rng_state = (uint64_t)seed * 0x5DEECE66DULL + 0xBULL;
    for (uint64_t i = n - 1; i > 0; --i) {
        uint64_t j = rand_xor_shift64(&rng_state) % (i + 1);
        // Swap
    ...
    printf("  Shuffle indices generated for %llu rows\n", (unsigned long long)n);
    return indices;
}
...
static void write_row(Buffer *buf, const Config *cfg, const IdPlan *plan,
                      uint64_t row_idx, uint32_t seed_base) {
    char idbuf[64];
    char randbuf[512];
    ...
    for (int c = 0; c < cfg->columns; ++c) {
        ...
        if (col->is_identifier) {
            ...
            // Measure: generate random value
            uint32_t s = seed_base ^ (uint32_t)row_idx ^ (uint32_t)(c * 0x9E37);
            ...
            switch (col->type_code) {
                ...
                case TYPE_NUMBER: {
                    double u = (double)rand_xor_shift(&s) / 4294967296.0;
...
int main(int argc, char **argv) {
    ...
    printf("Generating CSV (C): '%s'\n", cfg.csv_path);
    printf("  Rows: %llu | Chunks: %llu | Shuffle: %s\n",
           (unsigned long long)cfg.rows,
           (unsigned long long)cfg.chunk_size,
           cfg.shuffle ? "yes" : "no");

    // Generate shuffle indices if needed
    uint64_t *shuffle_indices = NULL;
    if (cfg.shuffle) {
        shuffle_indices = generate_shuffle_indices(cfg.rows, cfg.seed);
        if (!shuffle_indices) {
            free_id_plan(&plan);
            return 5;
        }
    ...
    if (!out) {
        perror("Open csv");
        free_id_plan(&plan);
        if (shuffle_indices) free(shuffle_indices);
        return 4;
    }
    setvbuf(out, NULL, _IOFBF, 16 * 1024 * 1024);  // 16MB buffer

    write_header(out, &cfg);

    Buffer buf;
    buf_init(&buf, 1 << 20);  // 1MB initial buffer

    uint32_t seed_base = cfg.seed;
    uint64_t written = 0;
    ...
    while (written < cfg.rows) {
        ...
        for (uint64_t r = 0; r < cur; ++r) {
            uint64_t row_idx;
            if (shuffle_indices) {
                row_idx = shuffle_indices[written + r];
            } else {
                row_idx = written + r;
            }
            write_row(&buf, &cfg, &plan, row_idx, seed_base);
        }
