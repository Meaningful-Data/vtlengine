...
centralised_messages = {
    ...
    "0-1-2-8": "Cannot pass as inputs datasets/scalars defined as outputs of transformations in the script, please check: {names}",
    ...
    "0-3-1-4": "On Dataset {name} loading:  An identifier cannot have null values, found null values on {null_identifier}.",
    "0-3-1-5": "On Dataset {name} loading: Datasets without identifiers must have 0 or 1 datapoints.",
    ...
    "0-3-1-8": "On Dataset {name} loading: Duplicated identifiers are not allowed, found on row {row_index}",
    ...
    # General Operators
    "2-1-12-1": "At op {op}: Create a null measure without a scalar type is not allowed.Please use cast operator.", # RunTimeError.
    ...
    "2-1-15-1": "At op {op}: Component {comp_name} from dataset {dataset_name} contains negative values.", #RunTimeError
    ...
    "1-1-19-11": "The time aggregation operand has to be defined if not used inside an aggregation.",
    ...
    "2-1-19-13": "At op {op}: Invalid param data_type {type} for param {name}, expected {expected}.",
    "2-1-19-14": "At op {op}: Invalid dataset {name}, requires at least one Date/Time_Period measure.",
    ...
    "2-1-19-18": "At op {op}: Time operators do not support < and > comparison operations, so its not possible to use get the max or min between two time operators",
    "2-1-19-19": "Time Period comparison (>, <, >=, <=) with different period indicator is not supported, found {value1} {op} {value2}",
    "2-1-19-20": "Time Period operands with different period indicators do not support < and > comparison operations, unable to get the {op}",
