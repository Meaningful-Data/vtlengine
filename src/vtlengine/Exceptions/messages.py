...
centralised_messages = {
    ...
    "0-1-1-1": "invalid script format type: {format_}. Input must be a string, TransformationScheme"
    " or Path object",
    ...
    "0-1-1-9": "You have a problem related with commas, review rfc4180 standard, "
    "review file {file}.",
    ...
    "0-3-1-2": "{file} file not found. Please verify that the file exists and the provided"
    " path is correct.",
    "0-3-1-3": "Output folder {folder} not found or invalid. Must be a valid Path or S3 directory.",
    "0-3-1-4": "On Dataset {name} loading:  An identifier cannot have null values, found "
    "null values on {null_identifier}.",
    "0-3-1-5": "On Dataset {name} loading: Datasets without identifiers "
    ...
    "1-1-1-15": "At op {op}: Datasets {name_1} and {name_2} does not contain "
    ...
    "1-1-5-5": "A mask can't be provided to cast from type {type_1} to {type_2}. "
    ...
    # Clause errors
    "1-1-6-2": "At op {op}: The identifier {name} in dataset {dataset} could not be "
    "included in the {op} op.",
    "1-1-6-4": "At op {op}: Alias symbol cannot have the name of a "
    ...
    "1-1-6-8": "Cannot use component {comp_name} for rename, it is already "
    "in the dataset {dataset_name}.",
    ...
    # Comparison errors
    "1-1-7-1": "At op {op}: Value in {left_name} of type {left_type} "
    "is not comparable to value {right_name} of type {right_type}.",
    # Conditional errors
    "1-1-9-1": "At op {op}: The evaluation condition must result in a Boolean expression, "
    "found '{type}'.",
    "1-1-9-3": "At op {op}: Then clause {then_name} and else clause {else_name}, "
    ...
    "1-1-9-6": "At op {op}: Then-else datasets have different number of identifiers "
    "compared with condition dataset.",
    "1-1-9-9": "At op {op}: {clause} component {clause_name} role must be {role_1}, "
    "found {role_2}.",
    "1-1-9-10": "At op {op}: {clause} dataset have different number of identifiers "
    "compared with condition dataset.",
    "1-1-9-11": "At op {op}: Condition component {name} must be Boolean, found {type}.",
    "1-1-9-12": "At op {op}: then clause {then_symbol} and else clause {else_symbol}, both must be "
    "Datasets or at least one of them a Scalar.",
    "1-1-9-13": "At op {op}: then {then} and else {else_clause} datasets must contain "
    ...
    "1-1-10-3": "At op {op}: Name in the call {found} has to be equal to variable rule "
    "in signature {expected}.",
    "1-1-10-4": "At op {op}: When a hierarchical ruleset is defined for value domain, "
    ...
    "1-1-10-6": "At op {op}: Name in the call {found} has to be equal to variable condition "
    "in signature {expected} .",
    ...
    "Please use cast operator.",  # RunTimeError.
    # Join Operators
    "1-1-13-1": "At op {op}: Duplicated alias {duplicates}.",
    "1-1-13-2": "At op {op}: Missing mandatory aliasing.",
    "1-1-13-3": "At op {op}: Join conflict with duplicated names for column {name} "
    ...
    "1-1-13-5": "At op {op}: Invalid subcase B1, All the datasets must share as identifiers "
    "the using ones.",
    "1-1-13-6": "At op {op}: Invalid subcase B2, All the declared using components "
    "'{using_components}' must be present as components "
    "in the reference dataset '{reference}'.",
    "1-1-13-7": "At op {op}: Invalid subcase B2, All the non reference datasets "
    "must share as identifiers the using ones.",
    ...
    "1-1-13-11": "At op {op}: Invalid subcase A, {dataset_reference} should be a "
    "superset but {component} not found.",
    "1-1-13-12": "At op {op}: Invalid subcase A. There are different identifiers for "
    "the provided datasets",
    "1-1-13-13": "At op {op}: Invalid subcase A. There are not same number of identifiers "
    "for the provided datasets",
    "1-1-13-14": "Cannot perform a join over a Dataset Without Identifiers: {name}.",
    "1-1-13-15": "At op {op}: {comp_name} has to be a Measure for all the provided datasets "
    "inside the join",
    "1-1-13-16": "At op {op}: Invalid use, please review : {msg}.",
    "1-1-13-17": "At op {op}: {comp_name} not present in the dataset(result from join VDS) "
    "at the time it is called",
    # Operators general errors
    "1-1-14-1": "At op {op}: Measure names don't match: {left} - {right}.",
    "1-1-14-3": "At op {op}: Invalid scalar types for identifiers at DataSet {dataset}. "
    ...
    "1-1-14-9": "At op {op}: {names} with type/s {types} is not compatible with {op} "
    "on datasets {datasets}.",
    # Numeric Operators
    "1-1-15-8": "At op {op}: {op} operator cannot have a {comp_type} as parameter.",
    "2-1-15-1": "At op {op}: Component {comp_name} from dataset {dataset_name} contains "
    "negative values.",  # RunTimeError
    "2-1-15-2": "At op {op}: Value {value} could not be negative.",  # RunTimeError
    "2-1-15-3": "At op {op}: Base value {value} could not be less or equal 0.",  # RunTimeError
    "2-1-15-4": "At op {op}: Invalid values in Component {name}.",  # RunTimeError
    "2-1-15-5": "At op {op}: Invalid values in {name_1} and {name_2}.",  # RunTimeError
    "2-1-15-6": "At op {op}: Scalar division by Zero.",  # RunTimeError
    "2-1-15-7": "At op {op}: {op} operator cannot be a dataset.",  # RunTimeError
    # Set Operators
    "1-1-17-1": "At op {op}: Datasets {dataset_1} and {dataset_2} have different "
    ...
    "1-1-19-5": "At op {op}: periodIndTo parameter must be a larger duration value "
    "than the values to aggregate.",
    "1-1-19-6": "At op {op}: Time type used in the component {comp} is not supported.",
    "1-1-19-7": "At op {op}: can be applied only on Data Sets (of time series) and returns a "
    ...
    "1-1-19-11": "The time aggregation operand has to be defined if not used inside an "
    "aggregation.",
    ...
    "1-3-24": "Internal error on Analytic operators inside a calc, No partition or order "
    "symbol found.",
    ...
    "least: {expected}",
    "1-3-29": "At op {op}: can not use user defined operator that returns a component "
    "outside clause operator or rule",
    ...
    "1-4-1-5": "Found duplicates at rule naming: {names}. Please review {type} {ruleset_name} "
    "definition.",
    ...
    "definition {name}.",
    "1-4-1-8": "All rules must have different code items in the left side of '=' in "
    "hierarchy operator at hierachical ruleset definition {name}.",
    "1-4-1-9": "At op check_datapoint: {name} has an invalid datatype expected DataSet, "
    "found Scalar.",
    # AST Creation
    ...
    "1-4-2-4": "At op {op}: Only one order_by element must be used in Analytic with "
    "range windowing.",
    "1-4-2-5": "At op {op}: User defined operator without returns is not implemented.",
    "1-4-2-6": "At op {op}: Window must be provided.",
    "1-4-2-7": "At op {op}: Partition by or order by clause must be provided for "
    "Analytic operators.",
    "1-4-2-8": "At op {op}: Vtl Script contains Cycles, no DAG established. "
    "Nodes involved: {nodes}.",
    # ---------- RunTimeErrors ----------
    "2-1-19-1": "At op {op}: Invalid values {value_1} and {value_2} for duration, periodIndTo "
    ...
    "2-1-19-7": "Period Number must be between 1 and {periods} for "
    ...
    "expected {expected}.",
    "2-1-19-14": "At op {op}: Invalid dataset {name}, requires at least one "
    ...
    "2-1-19-17": "At op {op}: Time operators comparison are only support = and "
    ...
    "2-1-19-19": "Time Period comparison (>, <, >=, <=) with different period indicator "
