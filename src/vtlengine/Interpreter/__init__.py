...
from vtlengine.DataTypes import (
    ...
    Boolean,
    ScalarType,
    check_unary_implicit_promotion, binary_implicit_promotion,
)
...
class InterpreterAnalyzer(ASTTemplate):
    ...
    def visit_If(self, node: AST.If) -> Dataset:
        ...
        if isinstance(condition, Scalar):
            ...
            return thenValue if condition.value else elseValue

        # Lazy condition: return result after evaluating with lazy DuckDB logic
        then_dataset = self.visit(node.thenOp)
        else_dataset = self.visit(node.elseOp)

        return If.analyze(condition, then_dataset, else_dataset)
