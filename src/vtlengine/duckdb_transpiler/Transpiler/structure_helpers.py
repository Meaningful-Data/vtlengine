"""
Structure and resolution helpers for the SQL Transpiler.

This mixin class provides all methods related to:
- Operand type determination (dataset vs scalar vs component)
- Dataset structure resolution and building
- UDO parameter handling
- Component name resolution
- SQL literal conversion
- Time/group column resolution

These methods are used by the main SQLTranspiler but don't generate
top-level SQL queries themselves — they resolve metadata, types, and
structures that the SQL-generating visitors depend on.
"""
...
class StructureHelpersMixin:
    """Mixin providing structure resolution and helper methods for SQLTranspiler.

    All fields referenced via ``self.*`` are defined on the SQLTranspiler
    dataclass that inherits from this mixin.
    """
