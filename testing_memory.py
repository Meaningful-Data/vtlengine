...
def monitor_memory(pid, stop_event, peak_rss_holder, peak_duck_holder, peaks_log, conn, check_interval=1.0):
    ...
    while not stop_event.is_set():
        ...
        if mem_rss > peak_rss_holder[0] or mem_duck > peak_duck_holder[0]:
            ...
            print(f"[{timestamp:.2f}s] New peak -> RSS: {mem_rss/(1024**2):.2f} MB | DuckDB: {mem_duck/(1024**2):.2f} MB")
...
def execute_test(csv_path: Path, ds_path: Path, script: str, base_memory_limit: str, output_folder: Path):
    ...
    monitor_thread = threading.Thread(
        ...
        args=(os.getpid(), stop_event, peak_rss_holder, peak_duck_holder, peaks_log, conn,  1.0),
        daemon=True
    )
...
def save_results(file_csv, file_json, mem_limit, duration_sec, peak_rss_mb, peak_duck_mb, output_files, peaks_list):
    file_exists = RESULTS_FILE.exists()
    with open(RESULTS_FILE, mode="a", newline="") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow([
                "Date", "CSV", "JSON", "Memory Limit", "Duration (s)",
                "Peak RSS (MB)", "Peak DuckDB (MB)", "Output Files", "Memory Peaks (t_s:RSS/DUCK_MB)"
            ])
...
if __name__ == "__main__":
    base_path = Path(__file__).parent / "data" / "BIG_DATA"
    csv_file = base_path / "dp" / "DS_2.csv"
    ds_file = base_path / "ds" / "DS_2.json"
    output_folder = base_path / "output"

    vtl_script = """
    DS_r <- DS_2 [calc Me_5 := Me_1 * 10];
    """
    execute_test(csv_file, ds_file, vtl_script, base_memory_limit="4GB", output_folder=output_folder)
