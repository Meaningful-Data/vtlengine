...
class AdditionalHelper(TestCase):
    ...
    def LoadInputs(cls, code: str, number_inputs: int) -> Dict[str, Dataset]:
        ...
        for i in range(number_inputs):
            json_file_name = str(
                cls.filepath_json / f"{code}-DS_{str(i + 1)}{cls.JSON}"
            )
    ...
    @classmethod
    def BaseTest(
        cls, text: str, code: str, number_inputs: int, references_names: List[str]
    ):
        """ """
        if text is None:
...
class JoinOperatorsTest(AdditionalHelper):
    ...
    def test_30(self):
        ...
        self.NewExceptionTest(
            text=text, code=code, number_inputs=number_inputs, message=message
        )
