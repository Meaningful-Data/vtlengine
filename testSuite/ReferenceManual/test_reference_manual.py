...
if os.environ.get("SPARK", False):
    ...
    os.environ["PYSPARK_SUBMIT_ARGS"] = (
        "--conf spark.network.timeout=600s pyspark-shell"
    )
else:
    import pandas as pd
    ...
    for f in dataStructures:
        ...
        for dataset_json in structures["datasets"]:
            ...
            if dataset_name not in dataPoints:
                ...
                data = pd.read_csv(
                    os.path.join(dp_dir, f"{param}-{dataset_name}.csv"), sep=","
                )
...
def test_reference_defined_operators(
    input_datasets, reference_datasets, ast_defined_operators, param
):
    input_datasets = load_dataset(*input_datasets, dp_dir=input_dp_dir, param=param)
    reference_datasets = load_dataset(
        *reference_datasets, dp_dir=reference_dp_dir, param=param
